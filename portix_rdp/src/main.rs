...
fn patch_framebuf(
    ...
    if w > 100 && h > 100 {
        ...
        if is_single_color {
            ...
            for row in 0..h {
                if y + row >= dst_h { break; }
                for col in 0..w {
                    if x + col >= dst_w { break; }
...
fn main() {
    ...
    if args.len() < 5 {
        eprintln!("Usage: portix-rdp <host> <port> <username> <password> [domain] [width] [height]");
        std::process::exit(1);
    }
    ...
    println!("  user    : {}{}", domain.as_deref().map(|d| format!("{}\\", d)).unwrap_or_default(), username);
    ...
    let (frame_tx_std, frame_rx_std) =
        std::sync::mpsc::sync_channel::<RdpFrameEvent>(512);
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            loop {
                if cancel_consumer.is_cancelled() { break; }
                ...
                // Patch ke back buffer
                {
                    ...
                    patch_framebuf(
                        ...
                        win_w, win_h,
                        &ev.data,
                        ev.x as usize, ev.y as usize,
                        ev.width as usize, ev.height as usize,
                    );
                }
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            rt.block_on(async move {
                ...
                let (bcast_error_tx, mut bcast_error_rx) =
                    broadcast::channel::<RdpErrorEvent>(64);
                ...
                let (cmd_async_tx, cmd_async_rx) =
                    tokio::sync::mpsc::channel::<RdpCommand>(256);
                let cc_cmd = cancel_tokio.clone();
                tokio::task::spawn_blocking(move || {
                    loop {
                        match cmd_rx.recv_timeout(Duration::from_millis(50)) {
                            Ok(cmd) => {
                                if cmd_async_tx.blocking_send(cmd).is_err() { break; }
                            }
                            Err(std::sync::mpsc::RecvTimeoutError::Timeout) => {
                                if cc_cmd.is_cancelled() { break; }
                            }
                            Err(_) => break,
                        }
                    }
    ...
    while window.is_open() && !cancel_win.is_cancelled() {
        ...
        if let Some((mx, my)) = window.get_mouse_pos(MouseMode::Clamp) {
            ...
            if lmb != prev_lmb {
                prev_lmb = lmb;
                let _ = cmd_tx_win.try_send(RdpCommand::MouseButton { x: ix, y: iy, button: 0, down: lmb });
            }
            if rmb != prev_rmb {
                prev_rmb = rmb;
                let _ = cmd_tx_win.try_send(RdpCommand::MouseButton { x: ix, y: iy, button: 2, down: rmb });
            }
            if mmb != prev_mmb {
                prev_mmb = mmb;
                let _ = cmd_tx_win.try_send(RdpCommand::MouseButton { x: ix, y: iy, button: 1, down: mmb });
            }
        }
        ...
        if let Some((_, sy)) = window.get_scroll_wheel() {
            if sy.abs() > 0.01 {
                ...
                let _ = cmd_tx_win.try_send(RdpCommand::MouseWheel {
                    x: prev_mx as u16, y: prev_my as u16,
                    delta, is_vertical: true,
                });
            }
        }
        ...
        for &key in &curr_keys {
            if !prev_keys.contains(&key) {
                if let Some(sc) = key_to_scancode(key) {
                    let _ = cmd_tx_win.try_send(RdpCommand::KeyboardInput { scancode: sc, down: true });
                }
            }
        }
        for &key in &prev_keys {
            if !curr_keys.contains(&key) {
                if let Some(sc) = key_to_scancode(key) {
                    let _ = cmd_tx_win.try_send(RdpCommand::KeyboardInput { scancode: sc, down: false });
                }
            }
        }
        prev_keys = curr_keys;
