...
void main() {
  group('TerminalPane integration', () {
    testWidgets(
      'drag selection works from upper area in EOF-like alt buffer pane',
      (tester) async {
    ...
    testWidgets('drag selection still works when centered only', (
      tester,
    ) async {
      final terminal = Terminal(maxLines: 200);
      final controller = TerminalController(
      ...
      expect(
        selection.normalized.end.y,
        greaterThan(selection.normalized.begin.y),
      );
    ...
    testWidgets('drag selection still works when using Positioned.fill only', (
      tester,
    ) async {
      final terminal = Terminal(maxLines: 200);
      final controller = TerminalController(
