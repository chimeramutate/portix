...
Future<void> _dragTabOntoTab(
  ...
  final draggedTab = find.byKey(
    ValueKey('terminal-session-tab-$draggedSessionId'),
  );
  final targetTab = find.byKey(
    ValueKey('terminal-session-tab-$targetSessionId'),
  );
...
void main() {
  group('Terminal session tab drag-and-drop', () {
    testWidgets(
      'dragging active tab onto another tab uses that tab as explicit target',
      (tester) async {
    ...
    testWidgets(
      'dragging inactive tab onto active tab keeps active tab as explicit target',
      (tester) async {
        final harnessKey = GlobalKey<_TabDragHarnessState>();
