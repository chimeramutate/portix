...
void main() {
  ...
  group('split target selection', () {
    test(
      'dragging active tab into workspace drop area keeps explicit target',
      () {
        ...
        expect(
          ...
          'session-3',
          reason:
              'active tab dropped onto an explicit pane target must merge with that target',
        );
      },
    );

    test(
      'dragging non-active tab onto active tab keeps active tab as target',
      () {
        const orderedIds = ['session-1', 'session-2', 'session-3'];
        ...
        expect(
          ...
          'session-2',
          reason:
              'dropping an inactive tab onto the active tab should create a workspace with the active tab, not the left neighbour',
        );
      },
    );

    test(
      'without explicit target the dragged tab falls back to left neighbour',
      () {
        const orderedIds = ['session-1', 'session-2', 'session-3'];
    ...
    test('order controller preserves the intended neighbour relationship', () {
      ...
      expect(ordered.map((session) => session.id).toList(), [
        'session-1',
        'session-2',
        'session-3',
      ]);
