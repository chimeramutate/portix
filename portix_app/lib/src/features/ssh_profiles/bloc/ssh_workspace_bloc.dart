...
class SshWorkspaceBloc extends Bloc<SshWorkspaceEvent, SshWorkspaceState> {
  SshWorkspaceBloc({required SshProfileRepository repository})
  ...
  final SshProfileRepository _repository;
  ...
  Future<void> _onProfileEditRequested(
    ...
    if (profile.authMethod == AuthMethod.password) {
      final realPassword = await _repository.readPasswordForEdit(profile.id);
  ...
  Future<void> _onProfileTestRequested(
    ...
    final result = await _repository.testConnection(profile);
    result.fold(
      (failure) =>
  ...
  Future<void> _onProfilesImported(
    ...
    for (final profile in event.profiles) {
      ...
      final saved = result.fold<SshProfile?>((_) => null, (profile) => profile);
  ...
  String? _profileValidationMessage(SshProfile profile) {
    ...
    if (profile.group.trim().isEmpty) return 'Profile group is required.';
