...
const _terminalControllerDebugEnabled = bool.fromEnvironment(
  'PORTIX_TERMINAL_DEBUG',
);

void _terminalControllerDebugLog(String message) {
  if (_terminalControllerDebugEnabled) {
    dev.log(message, name: 'portix.terminal.controller');
  }
}
...
class TerminalController with ChangeNotifier {
  ...
  void setSelection(CellAnchor base, CellAnchor extent, {SelectionMode? mode}) {
    ...
    final baseDebug = base.attached ? '${base.offset}' : 'detached(x=${base.x})';
    final extentDebug = extent.attached
        ? '${extent.offset}'
        : 'detached(x=${extent.x})';
    _terminalControllerDebugLog(
      'setSelection base=$baseDebug extent=$extentDebug mode=$_selectionMode',
    );
    notifyListeners();
  }
  ...
  void clearSelection() {
    ...
    _selectionExtent?.dispose();
    _selectionExtent = null;
    _terminalControllerDebugLog('clearSelection');
    notifyListeners();
  }
  ...
  void setSuspendPointerInput(bool suspend) {
    ...
    _terminalControllerDebugLog('setSuspendPointerInput suspend=$suspend');
    notifyListeners();
  }
  ...
  bool shouldSendPointerInput(PointerInput pointerInput) {
    // Don't send pointer events to terminal when there's an active selection.
    // This allows mouse drag selection to work in apps like less/vim.
    if (_selectionBase != null && _selectionExtent != null) {
      return false;
    }
